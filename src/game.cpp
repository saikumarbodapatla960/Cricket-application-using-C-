...
void Game::summary() {
	...
	cout << "\n\t\t MATCH ENDS! \t\t" << endl << endl
			<< "\n\t\t MATCH SUMMARY \t\t\n" << endl
			<< teamA.name << "  " << teamA.totalRunsScored << " - "
		    << teamA.wicketsLost << " (" << teamA.totalBallsBowled << ") " << endl
			<< " PLAYER  \t\t\t  BATTING  \t\t\t  BOWLING " << endl;

	for (int i = 0; i < playersPerTeam; i++) {
		cout << teamAPlayers[i].name << " \t\t" << teamAPlayers[i].runsScored << "("
				<< teamAPlayers[i].ballsPlayed << ")   \t\t"
				  << teamAPlayers[i].ballsBowled << " - " << teamAPlayers[i].runsGiven << " - "
				  << teamAPlayers[i].wicketsTaken << "\t" << endl ;
			};
	cout << endl << teamB.name << "  " << teamB.totalRunsScored << " - "
		    << teamB.wicketsLost << " (" << teamB.totalBallsBowled << ") " << endl
			<< " PLAYER  \t\t\t  BATTING  \t\t\t  BOWLING " << endl;
	for (int i = 0; i < playersPerTeam; i++) {
			cout << teamBPlayers[i].name << " \t\t" << teamBPlayers[i].runsScored << "("
				<< teamBPlayers[i].ballsPlayed << ")   \t\t"
				  << teamBPlayers[i].ballsBowled << " - " << teamBPlayers[i].runsGiven << " - "
